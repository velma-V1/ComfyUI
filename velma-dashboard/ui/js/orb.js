...
(function () {
  ...
  var FRAG = [
    ...
    "  return pow(noise(vec2(t * 0.8, seed * 3.7)), 6.0) * 3.2;",
    "}",
    "",
    ...
    "  float eb = 0.22 + 0.7 * u_energy;",
    "  float gG = strikeGate(t, 1.3)  * eb + u_audio * 0.9;",
    "  float gP = strikeGate(t, 4.2)  * eb + u_audio * 0.9;",
    "  float gB = strikeGate(t, 7.9)  * eb + u_audio * 0.9;",
    "  float gV = strikeGate(t, 11.6) * eb + u_audio * 0.9;",
    "  inner += (u_dyeG * u_weights.x * (fG.x * (0.24 + 0.55 * gG) + fG.y * gG * 0.22)",
    "          + u_dyeP * u_weights.y * (fP.x * (0.24 + 0.55 * gP) + fP.y * gP * 0.22)",
    "          + u_dyeB * u_weights.z * (fB.x * (0.24 + 0.55 * gB) + fB.y * gB * 0.22)",
    "          + u_dyeV * u_weightV  * (fV.x * (0.24 + 0.55 * gV) + fV.y * gV * 0.22)) * breath;",
    "",
    "  // ---- crossings between two colors flash hot ----",
    "  float inter = fG.x * fP.x + fG.x * fB.x + fG.x * fV.x",
    "              + fP.x * fB.x + fP.x * fV.x + fB.x * fV.x;",
    "  inner += mix(dyeMix, vec3(1.0), 0.32) * inter * (0.40 + 0.30 * (gG + gP + gB + gV));",
    "",
    ...
    "  float fl = (0.08 + 0.22 * slow + 1.1 * burst)",
    "           * (0.28 + 0.85 * u_energy) + u_audio * 0.6;",
    "  float strobe = 0.7 + 0.3 * noise(vec2(t * 2.2, 3.7));",
    "  float core = exp(-r * r * (95.0 - 55.0 * clamp(fl, 0.0, 1.0))) * fl * strobe;",
    "  inner += mix(dyeMix, vec3(1.0, 0.95, 1.0), 0.30) * core;",
    "",
    ...
    "  col *= u_dim;",
    "",
    "  // tone map (stronger highlight compression to tame overall",
    "  // brightness), then push saturation: intense color, restrained light",
    "  col = col / (1.0 + col * 1.35);",
